...
// containing the names of all people who 
...
let  NamesOfAdults = (people) => {
    ...
    let  adultNames = adults.map(adult => adult.name);
    return adultNames;
  }
...
// a product with a name, price, and category property.
...
let productsByCategory = (products) => {
  ...
  let products = [  { name: 'Laptop', price: 1200, category: 'Electronics' },  
  { name: 'Shirt', price: 25, category: 'Clothing' },  
  { name: 'Headphones', price: 80, category: 'Electronics' }, 
   { name: 'Shoes', price: 60, category: 'Clothing' },];
...
let  namesOfTopStudents = (students) => {
  let  topStudents = students.filter(student => {
    let sum = student.scores.reduce((accumulator, currentValue) => accumulator + currentValue);
    let average = sum / student.scores.length;
    return average >= 85;
  });
  let namesOfTopStudents = topStudents.map(topStudent => topStudent.name);
...
console.log(namesOfTopStudents(students));
...
//  The age method should return the current age of the car based on the current year and the car's year
//  property.
const car = {
  make: 'Ford',
  model: 'Ranger',
